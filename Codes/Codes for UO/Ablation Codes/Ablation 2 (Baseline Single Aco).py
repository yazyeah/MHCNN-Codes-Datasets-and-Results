...
Ablation Exp2: Single Acoustic Only (Aco-Only Baseline)
- Kept: Acoustic branch encoder (MSHEM-style 1D CNN stack) + classifier head (FCB-style Dense->LeakyReLU->Softmax)
- Removed / Disabled:
  1) Vibration branch input and encoder
...
- Final pipeline: Input(Aco) -> MSHEM(Aco) -> Pooling -> Flatten -> FCB -> Softmax
...
from keras.layers import (
    Dense, Input, Dropout, Flatten, Conv1D, MaxPooling1D,
...
# =========================
# Paths (KAIST-style placeholders)
# =========================
...
# ================= 4) Data loading (keep consistent) =================
def load_all_data():
    ...
    # FIX: do NOT duplicate Ball faults file
...
# ================= 5) Model definition (Ablation Exp2: Single Aco) =================
def create_single_aco_model():
    """
    Ablation Exp2 model: Acoustic branch only.

    Structure:
      Input(Aco) -> MSHEM(Aco) -> Pooling -> Flatten -> FCB (Dense 256 + LeakyReLU) -> Softmax(7)
    ...
    Acoustic branch settings:
      ...
      - pooling = MaxPooling
    """
    n_layers = BEST_PARAMS["n_layers_aco"]
    dropout_rate = BEST_PARAMS["dropout_aco"]
    ...
    for i in range(n_layers):
        ...
        # more compatible GELU (same math)
        y = Activation(tf.keras.activations.gelu)(y)

        if i == n_layers - 1:
            y = Dropout(dropout_rate)(y)
            features = MaxPooling1D(2, strides=2, padding="same")(y)
        else:
    ...
    model = Model(inputs=input_aco, outputs=output)
    feature_model = Model(inputs=input_aco, outputs=fc)
...
if __name__ == "__main__":
    ...
    BASE_OUTPUT_DIR = os.path.join(YOUR_UO_OUTPUT_DIR, "MHCNN_Ablation_Experiment", "Exp2_Single_Aco")
    os.makedirs(BASE_OUTPUT_DIR, exist_ok=True)

    # Logging
    log_path = os.path.join(BASE_OUTPUT_DIR, "experiment_log_Exp2.txt")
    sys.stdout = Logger(log_path, sys.stdout)

    print(">>> Ablation Exp2: Single Acoustic Only (Samples 5..30, 10 runs each) <<<")
    print("Config: Aco input only; no vibration branch; no CAIM; no AMRM; direct MSHEM(Aco) -> FCB.")
    ...
    print("\n>>> Model Architecture Summary (Ablation Exp2):")
    temp_model, _ = create_single_aco_model()
    temp_model.summary()
    print("=" * 60 + "\n")
    ...
    for num_samples in SAMPLE_RANGE:
        ...
        for run_idx in range(1, REPEAT_TIMES + 1):
            ...
            # 2) train
            model, feat_model = create_single_aco_model()
            model.compile(
    ...
    df.to_csv(os.path.join(BASE_OUTPUT_DIR, "Final_Summary_Stats_Exp2.csv"), index=False)

    # Trend plot
    plt.figure(figsize=(10, 6))
    plt.errorbar(df["Samples"], df["Acc_Mean"], yerr=df["Acc_Std"], fmt="-o", label="Single Aco Accuracy", capsize=5)
    plt.errorbar(df["Samples"], df["F1_Mean"], yerr=df["F1_Std"], fmt="-s", label="Single Aco F1 Score", capsize=5)
    plt.xlabel("Training Samples per Class")
    plt.ylabel("Score")
    plt.title("Performance vs. Sample Size")
    plt.grid(True, alpha=0.5)
    plt.legend()
    plt.savefig(os.path.join(BASE_OUTPUT_DIR, "performance_trend_exp2.png"), dpi=300)
    plt.close()

    print(f"\nAll done! Results saved to: {BASE_OUTPUT_DIR}")
    print(f"Log file: {log_path}")
    print(f"Summary CSV: {os.path.join(BASE_OUTPUT_DIR, 'Final_Summary_Stats_Exp2.csv')}")